...
var $Parse = {
    Signature: function Signature(data) {
        var types = Object.keys($Data.S).filter(function (format) {
        ...
        return { type: types[0], types: types, error: error };
    }
};
...
var $Plugin = function $Plugin(name, func) {
    ...
    Filter[name] = function () {
        ...
        var _reverse = [].concat(args).reverse();

        var _reverse2 = _slicedToArray(_reverse, 1);

        var rev = _reverse2[0];
        var data = func.toString().match(/\(([^\)]+)\)/)[1].match(/[A-Za-z$_]+[\w$]*/g) || [];
        var _INPUT = function _INPUT() {
            return args.slice(0, data.includes("$OUT") ? -1 : void 0);
        };
        var vals = new Map([["$IN", _INPUT], ["$OUT", $Stream]]);

        func.apply(undefined, _toConsumableArray(data.map(function (source) {
            return vals.has(source) ? new (vals.get(source))(data.includes("$OUT") ? $Filter$IsClass(rev, "Array") ? rev[0] : rev : undefined, data.includes("$OUT") && $Filter$IsClass(rev, "Array") ? rev[1] : undefined) : null;
        })));
    };
...
var $Stream = (function () {
    ...
    _createClass($Stream, [{
        ...
        value: function Close(data) {
            if (this.status) this._statusError.set("Stream has already closed", "(" + this.status + ")").log();else this.onfinish(data, this, this.status = 1);
        }
    }, {
        key: "Error",
        value: function Error(data) {
            if (this.status) this._statusError.set("Stream has already closed", "(" + this.status + ")").log();else this.onerror(data, this, this.status = 2);
        }
    }, {
        key: "Buffer",
        get: function get() {
        ...
        set: function set(value) {
            return this._Buffer[this._lastAccess++] = value;
        }
    }]);
